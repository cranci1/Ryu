//
//  ExternalVideoPlayerLat.swift
//  Ryu
//
//  Created by Francesco on 02/09/24.
//
...
class ExternalVideoPlayerLat: UIViewController, WKNavigationDelegate, GCKRemoteMediaClientListener {
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) { [weak self] in
            self?.simulateClickOnPlayButton()
        }
    }
    
    private func simulateClickOnPlayButton() {
        let jsCode = """
        ...
        webView?.evaluateJavaScript(jsCode) { [weak self] result, error in
            if error == nil {
                ...
                print("Error clicking the play button: \(error?.localizedDescription ?? "Unknown error")")
            }
        }
    }
    ...
    private func extractIframeSource() {
        ...
        webView?.evaluateJavaScript(jsCode) { [weak self] result, error in
            if let iframeSrc = result as? String, let url = URL(string: iframeSrc), error == nil {
                print("Iframe src URL found: \(iframeSrc)")
                self?.loadIframeContent(url: url)
            } else {
                print("Error extracting iframe: \(error?.localizedDescription ?? "Unknown error")")
    ...
    private func loadIframeContent(url: URL) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) { [weak self] in
            self?.simulateClickOnNewPage()
        }
    }
    
    private func simulateClickOnNewPage() {
        let jsCode = """
        ...
        if (videoElement) {
            ...
            console.log('Video element not found on new page');
        }
        """
        
        webView?.evaluateJavaScript(jsCode) { [weak self] result, error in
            if error == nil {
                print("Attempted to click video element.")
            } else {
                print("Error clicking video element: \(error?.localizedDescription ?? "Unknown error")")
            }
            
            // Retry mechanism to account for video loading delay
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                self?.retryVideoClick()
            }
        }
    }
    
    private func retryVideoClick() {
        let jsCode = """
        var videoElement = document.querySelector('video');
        if (videoElement) {
            console.log('Retry: Video element found');
            var clickEvent = document.createEvent('MouseEvents');
            clickEvent.initMouseEvent('click', true, true, window);
            videoElement.dispatchEvent(clickEvent);
        } else {
            console.log('Retry: Video element still not found');
        }
        """
        
        webView?.evaluateJavaScript(jsCode) { result, error in
            if error == nil {
                print("Retry: Attempted to click video element.")
            } else {
                print("Retry: Error clicking video element: \(error?.localizedDescription ?? "Unknown error")")
            }
        }
    }
...
extension ExternalVideoPlayerLat: CustomPlayerViewDelegate {
